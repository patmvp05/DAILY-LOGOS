...
export async function getChapterInfo(bookName: string, chapter: number): Promise<ChapterInfo> {
  ...
  try {
    ...
    if (bookName === 'Proverbs') {
      // Proverbs uses WEB (World English Bible) via bolls.life API.
      // There is NO local proverbs.json file — do not try to fetch one.
      const response = await fetch(`https://bolls.life/get-chapter/WEB/${bookId}/${chapter}/`, {

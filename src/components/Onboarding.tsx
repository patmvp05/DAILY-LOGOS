...
export function Onboarding({ onComplete }: { onComplete: (date: string) => void }) {
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-12 text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="w-20 h-20 bg-evernote/10 rounded-full flex items-center justify-center mx-auto mb-8">
        <Sparkles size={40} className="text-evernote" />
      </div>
      ...
      <div className="bg-[var(--audible-card)] border border-[var(--audible-border)] p-8 rounded-2xl shadow-xl mb-12">
        <div className="flex items-center gap-4 mb-6">
          <Calendar className="text-evernote" size={24} />
        ...
        <input 
          ...
          className="w-full bg-white dark:bg-zinc-900 border-2 border-[var(--audible-border)] rounded-xl p-4 text-xl font-bold focus:border-evernote focus:outline-none transition-colors mb-8"
        />

        <button 
          onClick={() => onComplete(new Date(date).toISOString())}
          className="w-full bg-evernote text-white font-black uppercase py-4 rounded-xl flex items-center justify-center gap-3 hover:bg-evernote/90 transition-all group active:scale-[0.98]"
        >
          Begin My Journey

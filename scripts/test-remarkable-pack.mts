...

{
  ...
  // Count what is actually on disk rather than asserting a fact about the
  // scraped data — which archive covers which day changes every night, and an
  // earlier version of this test hard-coded "insight-for-living has no future
  // days" and broke the moment that stopped being true.
  let expectedDevotionals = 0;
  const expectedMissing = new Set<string>();
  for (let d = 0; d < 7; d++) {
    const monthDay = format(addDays(START, d), 'MM-dd');
    for (const dev of INTERNAL_DEVOTIONALS) {
      if (readDevotional(dev.slug, monthDay)) expectedDevotionals++;
      else expectedMissing.add(dev.slug);
    }
  }
  eq('every devotional day on disk is in the pack', result.devotionalCount, expectedDevotionals);
  eq('and every gap is reported rather than silently dropped',
     result.missingDevotionals.join(','), [...expectedMissing].sort().join(','));
  ok('nothing outside the catalog is reported missing',
     result.missingDevotionals.every((s) => INTERNAL_DEVOTIONALS.some((d) => d.slug === s)),
     result.missingDevotionals.join(', '));

  // The check above goes vacuous whenever every archive happens to cover the
  // test week, which is most days — and a vacuous check caught nothing when the
  // reporting was deliberately broken. So find a week that genuinely has a gap
  // and assert the report there. Insight for Living is a rolling window of
  // recent days, so it can never hold all 366: a gap always exists.
  const insightDays = new Set(
    readdirSync(new URL('../public/devotionals/insight-for-living/', import.meta.url))
      .filter((f) => f.endsWith('.json')).map((f) => f.slice(0, 5))
  );
  ok('the rolling archive really is incomplete (else the next check is vacuous)',
     insightDays.size < 366, `${insightDays.size} days`);

  // Walk the calendar for a date whose whole following week Insight is missing.
  let gapStart: Date | null = null;
  for (let i = 0; i < 366 && !gapStart; i++) {
    const candidate = addDays(new Date(2026, 0, 1), i);
    const week = Array.from({ length: 7 }, (_, d) => format(addDays(candidate, d), 'MM-dd'));
    if (week.every((md) => !insightDays.has(md))) gapStart = candidate;
  }
  ok('found a week the rolling archive does not cover', gapStart !== null);

  if (gapStart) {
    const gapPack = await buildPack(fromStart, new Set(),
      { startDate: gapStart, days: 7, version: VERSION });
    ok('a gap in an archive is reported, not silently dropped',
       gapPack.missingDevotionals.includes('insight-for-living'),
       `reported: [${gapPack.missingDevotionals.join(', ')}]`);
    eq('and the other three still contribute every day', gapPack.devotionalCount, 21);
    eq('the pack is still complete', gapPack.chapterCount, 49);
    eq('and still valid', (await validateEpub(gapPack.epub)).length, 0);
  }

 ...
 *
 * Canonical book table shared by the Bible text scrapers. `id` matches
 * BOLLS_BIBLE_BOOK_IDS in src/constants.ts, which is also the directory name
 * under public/bible/{CODE}/{bookId}/, so these ids must never be renumbered.
 */
...
/**
 * Book names as they appear in source texts, mapped to this table's canonical
 * name. bereanbible.com writes "Psalm" (singular); the app uses "Psalms".
 */
...
/** Resolve a source book name to a canonical book record, or null. */
export function resolveBook(rawName) {
  const key = String(rawName).trim().toLowerCase().replace(/\s+/g, ' ');
  const canonical = NAME_ALIASES.get(key) || null;
  if (canonical) return BY_NAME.get(canonical.toLowerCase()) || null;
